...
class conv(operator):
    ...
    def forward(self, input, weights, bias):
        ...
        #####################################################################################
        # code here
        output_h = ((in_height + pad - kernel_h) // stride + 1)
        output_w = ((in_width + pad - kernel_w) // stride + 1)
        
        input_pad = np.zeros([batch, in_channel, in_height + pad, in_width + pad])
        input_pad[:,:,pad//2:in_height+1,pad//2:in_width+1] = input[:,:,:,:]
        
        X = np.empty((in_channel*kernel_h*kernel_w, batch*output_h*output_w))
        for b in range(batch):
            for r_h in range(output_h):
                for r_w in range(output_w):
                    X[:, b*output_h*output_w+r_h*output_w+r_w] = input_pad[b,:,r_h*stride:r_h*stride+kernel_h,r_w*stride:r_w*stride+kernel_w].reshape(1,-1)
        
        w = weights.reshape(out_channel,in_channel*kernel_h*kernel_w)
        
        output = np.dot(w, X).reshape(out_channel, batch, output_h, output_w).transpose([1,0,2,3])
        output = self.add_bias.forward(output.reshape(batch, -1), bias.repeat(output_h*output_w)).reshape(batch, out_channel, output_h, output_w)
        #####################################################################################
        return output
...
class pool(operator):
    ...
    def forward(self, input):
        ...
        #####################################################################################
        # code here
        output_h = ((in_height + pad - pool_height) // stride + 1)
        output_w = ((in_width + pad - pool_width) // stride + 1)
        
        input_pad = np.empty([batch, in_channel, in_height + pad, in_width + pad])
        input_pad[:] = np.nan
        input_pad[:,:,pad//2:in_height+1,pad//2:in_width+1] = input[:,:,:,:]
        
        X = np.zeros((pool_height*pool_width, batch*in_channel*output_h*output_w))
        for b in range(batch):
            for c in range(in_channel):
                for r_h in range(output_h):
                    for r_w in range(output_w):
                        X[:, b*in_channel*output_h*output_w+c*output_h*output_w+r_h*output_w+r_w] = input_pad[b,c,r_h*stride:r_h*stride+pool_height,r_w*stride:r_w*stride+pool_width].reshape(1,-1)
        
        output = None
        
        if pool_type == 'max':
            output = np.nanmax(X, axis=0).reshape(batch, in_channel, output_h, output_w)
        elif pool_type == 'avg':
            output = np.nanmean(X, axis=0).reshape(batch, in_channel, output_h, output_w)
...
class dropout(operator):
    ...
    def backward(self, out_grad, input):
        ...
        if self.training:
            ...
            in_grad = out_grad * scale * self.mask
            #####################################################################################
        else:
            in_grad = out_grad
        return in_grad
